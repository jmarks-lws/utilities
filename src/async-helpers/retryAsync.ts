 ...
 * @param options.delayFirstAttempt - whether the first attempt should be delayed. Typically `true`
 * @param options.timeout - time in milliseconds before making each attempt
 * @param options.attempts - total number of attempts before final failure
...
export const retryAsync = async <T extends unknown>(
  fn: () => Promise<T>,
  options: IRetryAsyncOptions = retryAsyncDefaultOptions,
): Promise<T> => {
  const _options = {
